package com.jonerds.ballanweather.utils;

public class AppConstants {

    public static final String SHARED_PREFS_NAME = "BALLAN_WEATHER";
    public static final String LAST_OPENED_CITY = "LAST_OPENED_CITY";

    public static final String CITY_ID = "CITY_ID";

    public static final String BASE_URL = "http://api.worldweatheronline.com/premium/v1/";
    public static final String API_KEY = "fc5c2bdca30145d6904180920190203";
    public static final String IS_FIRST_RUN = "firstrun";
    public static final String API_RESPONSE_JSON_FORMAT = "json";
    public static final String LANGUAGE = "en";
    public static final String FALSE = "no";
    public static final String TRUE = "yes";
    public static final String TAG = "DEBUG";
    public static final String CITY_TAG = "city";
    public static final String KEY_PARAM = "key";
    public static final String CITY_PARAM = "q";
    public static final String FORMAT_PARAM = "format";
    public static final String NORMAL_WEATHER_PARAM = "fx";
    public static final String NUMBER_OF_DAYS_PARAM = "num_of_days";
    public static final String CURRENT_CONDITIONS_PARAM = "cc";
    public static final String MONTHLY_CLIMATE_PARAM = "mca";
    public static final String SHOW_COMMENTS_PARAM = "show_comments";
    public static final String TIME_INTERVAL_PARAM = "tp";
    public static final String LANGUAGE_PARAM = "lang";
    public static final String TIME_ZONE_PARAM = "timezone";
    public static final String ONLY_POPULAR_PARAM = "popular";
    public static final String NUMBER_OF_SEARCH_RESULTS_PARAM = "num_of_results";

    public static final int NUMBER_OF_DAYS_TO_GET = 8;
    public static final int TIME_INTERVAL_TO_GET = 1;
    public static final int NUMBER_OF_SEARCH_ITEMS_TO_GET = 5;
    public static final int DATABASE_VERSION = 1;
    public static final int NEW_CITY_CODE = 99;
}
